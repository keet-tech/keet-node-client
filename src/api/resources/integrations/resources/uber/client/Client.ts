...
export class Uber {
    ...
    public async createSession(requestOptions?: Uber.RequestOptions): Promise<Keet.common.CreateSessionResponse> {
        const _response = await core.fetcher({
            url: urlJoin(environments.KeetEnvironment.Production, "/v1/uber/session"),
            method: "POST",
            headers: {
    ...
    /**
     * @param {Keet.integrations.OrderRide} request
     * @param {Uber.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Keet.common.UnAuthorizedError}
     * @throws {@link Keet.common.InternalServerError}
     ...
     *     })
     */
    public async orderRide(
        request: Keet.integrations.OrderRide,
        requestOptions?: Uber.RequestOptions
    ): Promise<Keet.integrations.OrderRideResponse> {
        const _response = await core.fetcher({
            url: urlJoin(environments.KeetEnvironment.Production, "/v1/uber/ride"),
            method: "POST",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
                "X-Account-Token":
            ...
            contentType: "application/json",
            requestType: "json",
            body: serializers.integrations.OrderRide.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
            maxRetries: requestOptions?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return serializers.integrations.OrderRideResponse.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                breadcrumbsPrefix: ["response"],
